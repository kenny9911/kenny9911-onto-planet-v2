...
In the foundation, the `invocation-boundary` package authenticates through an injected identity port, resolves reviewed agent/tool pins, filters tools by grants, constructs the run identity, and derives action targets from an active binding. It rejects caller-supplied authority fields. API deployment must expose this trusted boundary rather than the internal `AgentRuntime.run` or `ActionGateway.createIntent` methods. The included identity and checkpoint stores are fixture/in-memory ports; enterprise IdP integration, durable storage, and workload/client/source grant intersection are later gates.
...
The harness executes a bounded plan/observe/replan loop. It discovers only tools allowed by the pinned AgentSpec, current principal, tenant policy, and live source grants. It requests authorized Context Packs, gives the model the minimum required context, parses typed outputs, validates each proposed effect, and checkpoints after every effect and human wait. The model port can be changed without changing action semantics.
